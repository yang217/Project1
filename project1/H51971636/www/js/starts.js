window.onload=function(){
	console.log(1)
	var stitem=document.getElementsByClassName("stitem");
	var stbtns=document.getElementsByClassName("stbtn");
	var cons=document.getElementsByClassName("cons");
	var len=stbtns.length;
		console.log(stbtns)
	for(i=0;i<len;i++){
		stbtns[i].index=i;
	
		stbtns[i].onclick=function(){
			if(cons[this.index].innerHTML=="-"){
				cons[this.index].innerHTML="+"
				stitem[this.index].className="stitem active"
			}else{
				cons[this.index].innerHTML="-";
				stitem[this.index].className="stitem"
			}
